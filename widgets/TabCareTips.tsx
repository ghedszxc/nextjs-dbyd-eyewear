      ...
      <div className="mx-auto max-w-5xl xl:max-w-full">
        <Tabs defaultValue={tabs[0].tabName}>
          <div ref={tabsListContainerRef} className="relative lg:pb-20">
            <TabsList
              variant="line"
              className="mb-1 flex w-full justify-start gap-20 py-10 overflow-x-auto scrollbar-hide"
            >
              {tabs.map((tab) => (
                <TabsTrigger
                  key={tab.tabName}
                  value={tab.tabName}
                  className="font-matter-regular flex-none px-0 pb-2 text-lg text-black hover:after:opacity-100 data-[state=active]:text-black data-[state=active]:font-bold"
                >
                  {tab.tabName}
                </TabsTrigger>
              ))}
              ...
              <TabsContent key={tab.tabName} value={tab.tabName}>
                <div className="grid grid-cols-1 gap-0 bg-[#F6F6F6] p-6 lg:grid-cols-2 lg:gap-8 lg:p-20 mb-8">
                  <div className="flex flex-col items-start gap-[18px] lg:gap-8 pt-6 sm:pt-12 lg:flex-row lg:pt-6">
                    ...
                    <h3 className="font-matter-regular text-2xl lg:text-3xl text-[#000000]">
                      {tab.tabName}
                    </h3>
                  </div>
                  ...
                  <div className="flex flex-col justify-start pt-6 sm:pt-12 lg:pt-6 gap-4">
                    {tab.tabContent?.doc && (
                      <div className="[&_ul]:list-disc [&_ul]:pl-5 [&_li]:mb-4 lg:[&_li]:mb-1 [&_p]:mb-[24px]  lg:[&_p]:mb-[24px]">
                        <RichText
                          doc={{
                          ...
                          className={{
                            p: "font-matter-regular text-black text-sm lg:text-base leading-relaxed mb-2",
                          }}
                        />
                      </div>
                    )}
                    ...
                    {tab.cta?.href && (
                      <Link
                        href={tab.cta.href}
                        className="font-matter-regular mt-2 inline-block w-fit rounded-full bg-black px-6 py-3 text-sm text-white transition-colors hover:bg-gray-800"
                      >
