...
const PLPAdapter = async (widgets: any, lang: Language) => {
  ...
  const filters = widgets?.find((widget: any) => widget?.contents?.[0]?.name === "02_SideFilters");
  ...
  const topFilters = widgets?.find((widget: any) => widget?.contents?.[0]?.name === "06_TopFilters");
  ...
  const PLPData = {
    ...
    topFilters: safeJsonParse(topFilters?.contents[0]?.content?.local_settings?.code)?.topFilters || [],
    ...
    campaigns: await Promise.all(
      campaignStories.map(async (story: Story) => {
        ...
        const campaignSet: CampaignSet = {
          scope: story?.content?.collection_subtitle || "",
          banners:
            (await Promise.all(
                  ...
                  side: b?.content?.layout_variant?.includes("left")
                    ...
                    : b?.content?.layout_variant?.includes("right")
                      ...
                      : b?.content?.layout_variant?.includes("center")
                        ? "center"
                        : localSettings?.side ? localSettings.side.toLowerCase() : undefined,
                  desktopImage: {
