...
export type ProductCardProps = {
  ...
  lastInRow?: boolean;
  /** Rendered width hint — owned by the parent that owns the columns. */
  sizes?: string;
};
...
export function ProductCard(props: ProductCardProps) {
  ...
  const cardSizes = props.sizes ?? imageSizes({ base: "50vw", md: "25vw" });
      ...
      <Carousel setApi={setApi} className="flex h-auto flex-col justify-between">
        <CarouselContent className="h-full">
          {products.map((p, i) => {
            ...
            const type = p.productType.toLowerCase().replace(/\s+/g, "-");

            return (
              <CarouselItem key={`${p.name}-${i}`} className="h-full">
                <Link href={`/${category}/${type}/${slug}`}>
                  <div className={props.imageClassName ?? "flex h-[9999px] max-h-[177px] w-full items-center justify-center lg:max-h-[344px]"}>
        ...
        <div className={`flex justify-between gap-2 ${totalColors > 1 ? "flex-col" : "flex-col-reverse"}`}>
          ...
          <div
            className={`items-center transition-opacity duration-300 lg:grid lg:grid-cols-3 ${totalColors > 1 ? "opacity-100" : "opacity-0"}`}
          >
