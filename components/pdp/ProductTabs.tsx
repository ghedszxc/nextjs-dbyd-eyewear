...
import {
  TAB_LIST_CLASSES,
  TAB_TRIGGER_CLASSES,
  TAB_CONTENT_DESCRIPTION_CLASSES,
  TABS_CONTAINER_CLASSES,
  TAB_CONTENT_CARE_CLASSES,
} from "./constants";
...
export function ProductTabs({ description, details, care }: ProductTabsProps) {
  ...
  return (
    <Tabs defaultValue={availableTabs[0]?.id} className={TABS_CONTAINER_CLASSES}>
      ...
      {/* Tab Contents */}
      {description && (
        <TabsContent value="description" className={TAB_CONTENT_DESCRIPTION_CLASSES}>
          <RichText
            doc={{
      ...
      {care && (
        <TabsContent value="care" className={TAB_CONTENT_CARE_CLASSES}>
          <ul className="max-w-[624px] space-y-1">
            {care.map((item: string, index: number) => (
              <li key={index} className="flex items-start gap-2 leading-[150%]">
                <span className="shrink-0 text-base leading-[150%]">•</span>
                <span>{item}</span>
              </li>
            ))}
          </ul>
