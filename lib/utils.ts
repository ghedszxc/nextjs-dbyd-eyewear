 ...
 * 
 * CLIENT-SIDE (browser): Returns empty string for relative paths to avoid CORS across subdomains
 * SERVER-SIDE: Returns absolute URL (required for server-to-server API requests)
 * DEVELOPMENT: Returns localhost:3000 in both cases
 * 
 * For server-side fetches that explicitly need absolute URLs, use getBaseUrlForServer() instead.
 */
export const getBaseUrl = () => {
  const isProd = process.env.NODE_ENV !== "development";
  
  // In production on the client-side, return empty string for relative paths
  // This makes fetch() use current origin, avoiding CORS across subdomains
...
/**
 * For server-side fetches that explicitly require absolute URLs.
 * Never returns empty string - always returns a full URL.
 * Use this in server components/adapters instead of getBaseUrl().
 */
  ...
  return (products ?? [])
    .map((p) => {
      ...
      const group: VariantGroup = variants.map((v: any) => {
        ...
        const rawStatus = String(attributes?.STATUS_N1_26 ?? "").trim();
        const status = rawStatus === "Best Seller" ? "Bestseller" : rawStatus; // Normalize "Best Seller" to "Bestseller"
        ...
        return {
          ...
          polarized: attributes?.POLARIZED === "Polarized",
          frame_color: attributes?.FRONT_FRAME_COLOR ?? "",
          productType:
